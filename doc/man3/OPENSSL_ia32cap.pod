...
stored internaly as ten 32-bit capability vectors and for simplicity 
...
=head3 Notable Capability Bits for LV0

The following are notable capability bits from logical vector 0 <lv0>
resulting from the following execution of CPUID:

=item CPUID.(EAX=01H).EDX
=item CPUID.(EAX=01H).ECX
...
=head3 Notable Capability Bits for LV1

The following are notable capability bits from logical vector 1 <lv1>
resulting from the following execution of CPUID:

=item CPUID.(EAX=07H,ECX=0H).EBX
=item CPUID.(EAX=07H,ECX=0H).ECX
...
=head3 Notable Capability Bits for LV2

The following are notable capability bits from logical vector 2 <lv2>
resulting from the following execution of CPUID:

=item CPUID.(EAX=07H,ECX=0H).EDX
=item CPUID.(EAX=07H,ECX=1H).EAX
...
=head3 Notable Capability Bits for LV3

The following are notable capability bits from logical vector 3 <lv3>
resulting from the following execution of CPUID:

=item CPUID.(EAX=07H,ECX=1H).EDX
=item CPUID.(EAX=07H,ECX=1H).EBX
...
=head3 Notable Capability Bits for LV4

The following are notable capability bits from logical vector 4 <lv4>
resulting from the following execution of CPUID:

=item CPUID.(EAX=07H,ECX=1H).ECX
=item CPUID.(EAX=24H,ECX=0H).EBX
...
=head3 OPENSSL_ia32cap environment variable
...
of the logical vectors described above. Each value is delimited by a 'B<:>'.

=over 4
C<B<OPENSSL_ia32cap>=<lv0>B<:><lv1>B<:><lv2>B<:><lv3>B<:><lv4>>
=back
...
B<OPENSSL_ia32cap>=B<:>B<:>0x0B<:>0x0B<:>
...
B<OPENSSL_ia32cap>=B<~>0x200000000000000B<:><B<~>0x20000000000
